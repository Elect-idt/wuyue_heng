...
// 每个外设成员（led_ops、usart_ops等）是该类外设的统一操作接口
// 外设大部分遵循RAII（资源获取即初始化），但systick等关键系统资源需要在Bsp_Init中提前初始化
//
...
//   led_ops_t/usart_ops_t/...    → C++中 抽象产品+具体产品 的合并
//                                   C++需要多个派生类(LED_STATUS/LED_DEBUG/...)，
//                                   C中用id枚举(led_id_e)在同一类型内区分不同实例
//   g_stm32f4_bsp_               → 具体工厂实例，函数指针指向该平台的实现（类似填好的虚表）
//   g_stm32f4_led_driver_/...    → 具体产品实例，函数指针指向该外设的实现（类似填好的虚表）
//
//   因此无需简单工厂和工厂方法：
